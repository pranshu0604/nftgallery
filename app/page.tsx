...
export default function HomePage() {
  ...
  return (
    <>
      ...
      <main className={`flex min-h-screen flex-col items-center bg-[url('/bg-pattern.svg')] bg-fixed bg-cover bg-center bg-opacity-50 bg-white dark:bg-dark-background p-0 overflow-hidden ${pageLoading ? 'opacity-0' : 'opacity-100 transition-opacity duration-500'}`}>
        {/* Decorative elements */}
      ...
      <header className="w-full backdrop-blur-md bg-white/70 dark:bg-dark-background/80 border-b border-light-border dark:border-dark-border sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between px-6 sm:px-8 py-6 md:py-5">
          <div className="flex items-center">
            <motion.div 
              className="mr-3 bg-gradient-to-br from-light-accent to-light-accentHover dark:from-dark-accent dark:to-dark-accentHover p-2 rounded-lg shadow-lg"
              whileHover={{ 
                scale: 1.05,
                rotate: [0, 5, -5, 0],
              }}
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-extrabold text-light-text dark:text-dark-text tracking-tight flex items-center">
                <span className="bg-clip-text text-transparent bg-gradient-to-r from-light-text to-light-subtext dark:from-dark-text dark:to-dark-subtext">Solana</span>
                <span className="text-light-accent dark:text-dark-accent ml-2">Gallery</span>
              </h1>
              <p className="text-sm text-light-subtext dark:text-dark-subtext font-medium hidden md:block">Discover remarkable NFT treasures</p>
            </div>
          </div>
          <div className="mt-4 md:mt-0 flex gap-4 items-center">
            <ThemeToggle />
            {isClient && <WalletMultiButton className="!bg-gradient-to-r !from-light-accent !to-light-accentHover hover:!from-light-accentHover hover:!to-light-accent !text-white !font-bold !py-2.5 !px-6 !rounded-xl !shadow-lg !transition" />}
          </div>
        </div>
      </header>
      ...
      <section className="relative w-full max-w-7xl mx-auto px-6 sm:px-8 py-12 md:py-20">
        <div className="flex flex-col items-center text-center">
          <motion.h2 
            className="text-3xl md:text-5xl font-bold text-light-text dark:text-dark-text mb-6"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
          >
            Explore Your <span className="text-light-accent dark:text-dark-accent">NFT Collection</span>
          </motion.h2>
          <motion.p 
            className="text-lg text-light-subtext dark:text-dark-subtext max-w-2xl mb-10"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      {/* Footer */}
      <footer className="w-full bg-white/80 dark:bg-dark-background/80 backdrop-blur-md border-t border-light-border dark:border-dark-border py-12">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 flex flex-col md:flex-row justify-between items-center">
          <div className="mb-6 md:mb-0">
            <div className="flex items-center justify-center md:justify-start">
              <div className="mr-3 bg-gradient-to-br from-light-accent to-light-accentHover dark:from-dark-accent dark:to-dark-accentHover p-1.5 rounded-lg shadow-lg">
            ...
            <p className="text-sm text-light-subtext dark:text-dark-subtext mt-2 text-center md:text-left">
          ...
          <div className="flex flex-col items-center md:items-end">
            <div className="flex space-x-4 mb-4">
              <a href="#" className="text-light-subtext dark:text-dark-subtext hover:text-light-accent dark:hover:text-dark-accent transition-colors">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
